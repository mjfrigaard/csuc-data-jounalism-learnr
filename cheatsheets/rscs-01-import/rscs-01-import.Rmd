...
If we have a dataset in our R environment we want to write to an Excel sheet, we can do this with either `writexl` or `openxlsx`. The `writexl` package is lightweight and only has a single function for writing data frames to excel, `write_xlsx()`. 
